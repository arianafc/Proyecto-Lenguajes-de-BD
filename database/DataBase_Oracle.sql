...
BEGIN
    ...
    SELECT 
        U.ID_USUARIO, 
        U.NOMBRE, 
        U.APELLIDO1 || ' ' || NVL(U.APELLIDO2, '') AS APELLIDOS,
        U.USERNAME, 
        U.EMAIL, 
...
CREATE OR REPLACE PROCEDURE SP_AGREGAR_USUARIO(
    ...
    V_ID_USUARIO NUMBER;
BEGIN
    SELECT SEQ_USUARIOS.NEXTVAL INTO V_ID_USUARIO FROM DUAL;
...
--SP para obtener el usuario para editar
...
BEGIN
    OPEN P_CURSOR FOR
    SELECT 
        ID_USUARIO, NOMBRE, APELLIDO1, APELLIDO2, EMAIL, USERNAME, 
        ID_ROL, ID_ESTADO
    FROM USUARIOS
    WHERE ID_USUARIO = P_ID_USUARIO;
EXCEPTION
    WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20007, 'ERROR AL OBTENER USUARIO POR ID: ' || SQLERRM);
END SP_OBTENER_USUARIO_POR_ID;
...
--SP para verificar si el usuario o el correo existen
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20003, 'ERROR AL VERIFICAR USUARIO: ' || SQLERRM);
END SP_VERIFICAR_USUARIO_EXISTENTE;
